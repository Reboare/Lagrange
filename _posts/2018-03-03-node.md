...
```
We're quite restricted on this binary.  ASLR is enabled (but not PIE), so we can't do the standard jump to an address.  Since PIE is disabled, we can just use any functions contained within the PLT however.
```
rabin2 -i backup
[Imports]
...
I hope you enjoyed this writeup!  If you have any feedback feel free to email, message or hit me on Slack!  Happy Hacking!
